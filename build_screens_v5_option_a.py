...
# -----------------------------------------------------------------------------
# SCREEN 0: APP CLUTTER HOOK SCREEN (3 Separate Currency Apps Problem Visual)
# -----------------------------------------------------------------------------
def build_v5_clutter():
    ...
    # Container Box representing phone screen home folder
    draw.rounded_rectangle([(40, 230), (1040, 2080)], radius=36, fill=(30, 41, 59, 240), outline=(239, 68, 68, 255), width=3)

    draw.text((220, 320), "MY MONEY APPS", fill=(248, 113, 113, 255), font=font_hero)
    draw.text((120, 410), "Managing multiple currencies before...", fill=(148, 163, 184, 255), font=font_mid)

    # App 1 Card: NGN Bank App
    draw.rounded_rectangle([(80, 520), (1000, 840)], radius=28, fill=(24, 34, 56, 255), outline=(239, 68, 68, 200), width=2)
    canvas.paste(flag_ngn, (120, 560), flag_ngn)
    draw.text((200, 560), "App 1: Local NGN Bank", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((120, 630), "₦ Naira Transfers Only", fill=(148, 163, 184, 255), font=font_mid)
    draw.rounded_rectangle([(760, 560), (960, 610)], radius=12, fill=(239, 68, 68, 255))
    draw.text((790, 570), "LIMITED", fill=(255, 255, 255, 255), font=font_xs)

    # App 2 Card: USD App
    draw.rounded_rectangle([(80, 880), (1000, 1200)], radius=28, fill=(18, 48, 38, 255), outline=(239, 68, 68, 200), width=2)
    canvas.paste(flag_usd, (120, 920), flag_usd)
    draw.text((200, 920), "App 2: Foreign USD Wallet", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((120, 990), "$ USD Dollar Only", fill=(148, 163, 184, 255), font=font_mid)
    draw.rounded_rectangle([(760, 920), (960, 970)], radius=12, fill=(239, 68, 68, 255))
    draw.text((790, 930), "LIMITED", fill=(255, 255, 255, 255), font=font_xs)

    # App 3 Card: GHS MoMo App
    draw.rounded_rectangle([(80, 1240), (1000, 1560)], radius=28, fill=(48, 40, 18, 255), outline=(239, 68, 68, 200), width=2)
    canvas.paste(flag_ghs, (120, 1280), flag_ghs)
    draw.text((200, 1280), "App 3: GHS MoMo Account", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((120, 1350), "GH₵ Cedi Payouts Only", fill=(148, 163, 184, 255), font=font_mid)
    draw.rounded_rectangle([(760, 1280), (960, 1330)], radius=12, fill=(239, 68, 68, 255))
    draw.text((790, 1290), "LIMITED", fill=(255, 255, 255, 255), font=font_xs)

    # Bottom Clutter Banner
    draw.rounded_rectangle([(80, 1650), (1000, 1780)], radius=24, fill=(239, 68, 68, 255))
    draw.text((220, 1690), "Too Many Apps & Fees ❌", fill=(255, 255, 255, 255), font=font_btn)

    path = os.path.join(screens_dir, "v5_clutter_hook.png")
    canvas.save(path)
    print(f"Saved {path}")

# -----------------------------------------------------------------------------
# SCREEN 1: MULTI-CURRENCY OVERVIEW (NoteStandard Solution)
# -----------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    build_v5_usd()
    build_v5_ghs()
    print("All Option A real NoteStandard screens generated successfully!")
