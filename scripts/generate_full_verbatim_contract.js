...
const {
  ...
  PageNumber, HeadingLevel, AlignmentType, WidthType, ImageRun, ShadingType
} = docx;
...
console.log('Generating Full Verbatim Uncondensed Executed Contract (.docx & .pdf)...');
...
async function generateFullDocx() {
  ...
  const doc = new Document({
    creator: 'Anchor Software Ltd & Jossy Digital Technologies Ltd',
    title: 'CLIENT SERVICE AGREEMENT - ANCHOR SOFTWARE LTD & JOSSY DIGITAL TECHNOLOGIES LTD',
    description: 'Full Executed Client Service Agreement',
    styles: {
      default: { font: 'Calibri', size: 22 }, // 11pt
      heading1: {
        run: { font: 'Calibri', size: 26, bold: true, color: '0D1B3D' },
        paragraph: { spacing: { before: 240, after: 120 } }
      },
      heading2: {
        run: { font: 'Calibri', size: 22, bold: true, color: '0052FF' },
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: 'CLIENT SERVICE AGREEMENT', size: 34, bold: true, color: '0D1B3D' })]
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 300 },
          children: [new TextRun({ text: 'BETWEEN\nANCHOR SOFTWARE LTD\nAND\nJOSSY DIGITAL TECHNOLOGIES LTD', size: 24, bold: true, color: '0052FF' })]
        }),
        ...
        new Paragraph({
          ...
          children: [
            ...
            new TextRun({ text: '1.1.26. Transaction Data means data acquired from the use of the service provided under this Agreement.', size: 22 })
          ]
        }),
        ...
        new Paragraph({
          alignment: AlignmentType.JUSTIFIED,
          children: [
            new TextRun({ text: '2.1.1. This Agreement shall commence from the Effective Date and remain in force for twelve (12) months ("Initial Term"), unless terminated in accordance with the provisions of this Agreement.\n2.1.2. Automatically renewed upon expiration of Initial Term for successive twelve (12) month periods unless terminated in accordance with this Agreement.', size: 22 })
          ]
        }),
        ...
        new Paragraph({
          alignment: AlignmentType.JUSTIFIED,
          children: [
            new TextRun({ text: '3.1. Anchor shall integrate with licensed financial institutions to provide full banking suites to the Client as indicated in Schedule A.\n3.2. Obligations governed by terms of Agreement and all attached schedules.', size: 22 })
          ]
        }),
        ...
        new Paragraph({
          alignment: AlignmentType.JUSTIFIED,
          children: [
            new TextRun({ text: '4.1. Licenses & Permits: Anchor is responsible for obtaining all relevant permits and licenses necessary to offer Subscribed Services.\n4.2. Software: Anchor provides API and Software. Material changes announced at least three (3) days in advance where reasonably possible.\n4.3. Account Support: Seamless technical specs, 24/7 service availability, and communication channels for inquiries.\n4.4. Disclosures & Security: Data processed under relevant data protection laws; 72-hour notice for any lawful account suspension.', size: 22 })
          ]
        }),
        ...
        // SCHEDULE A
        new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun('SCHEDULE A — SUBSCRIBED SERVICES & APPLICABLE FEES')] }),
        new Paragraph({
          alignment: AlignmentType.JUSTIFIED,
          spacing: { after: 120 },
          children: [new TextRun('Jossy Digital Technologies Ltd subscribes to the following production service modules under this Agreement:')]
        }),
        ...
        new Table({
          ...
          rows: [
            new TableRow({
              children: [
                new TableCell({ width: { size: 35, type: WidthType.PERCENTAGE }, shading: { fill: '0D1B3D', type: ShadingType.CLEAR }, children: [new Paragraph({ children: [new TextRun({ text: 'Service Category', bold: true, color: 'FFFFFF' })] })] }),
                new TableCell({ width: { size: 65, type: WidthType.PERCENTAGE }, shading: { fill: '0D1B3D', type: ShadingType.CLEAR }, children: [new Paragraph({ children: [new TextRun({ text: 'Subscribed Features & Production APIs', bold: true, color: 'FFFFFF' })] })] })
              ]
            }),
            new TableRow({
              children: [
        ...
        new Paragraph({
          alignment: AlignmentType.JUSTIFIED,
          spacing: { after: 200 },
          children: [new TextRun('IN WITNESS WHEREOF, the Parties have executed this Client Service Agreement as of the Effective Date written above.')]
        }),
              ...
              children: [
                ...
                new TableCell({
                  ...
                  children: [
                    ...
                    new Paragraph({ text: 'Operating Entity: NoteStandard' }),
                    ...
                    new Paragraph({ text: 'Phone: +2347051824027' }),
...
// Execute Generation
generateFullDocx().catch(err => console.error('Docx generation error:', err));
