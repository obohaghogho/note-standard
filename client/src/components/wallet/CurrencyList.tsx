                                    ...
                                    <div className="flex justify-between items-center gap-2">
                                        ...
                                        {wallet.asset !== 'USD' && (
                                            <div className="flex items-center gap-1.5 bg-black/20 px-2 py-0.5 rounded-full border border-white/10 backdrop-blur-md">
                                                <div className={`w-1.5 h-1.5 rounded-full ${wallet.mode === 'FRESH' ? 'bg-green-400' : 'bg-yellow-400'} animate-pulse shadow-[0_0_8px_rgba(74,222,128,0.6)]`} />
                                                <p className="text-[10px] font-black text-white whitespace-nowrap">
                                                    Market {wallet.mode === 'FRESH' ? 'LIVE' : 'LKG'}
                                                </p>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div>
                                ...
                                {hasLocked && (
                                    <p className="text-[10px] text-amber-300/80 mt-0.5 font-medium">
                                        {showBalances ? `Avail: ${wallet.available}` : 'Avail: ••••'}
                                    </p>
                                )}
                                {wallet.asset !== 'USD' && (
                                    <p className="text-xs text-white/50 mt-1">
                                        ≈ {showBalances ? wallet.valuationUsd : '••••'}
                                    </p>
                                )}
                            </div>

                            {/* Wallet Address for Crypto */}
                            {['BTC', 'ETH', 'USDT', 'USDC'].some(c => wallet.asset.startsWith(c)) && wallet.address && (
                                    ...
                                    <div className="flex items-center justify-between gap-2 bg-black/20 p-2 rounded-lg group/addr transition-colors hover:bg-black/30">
                                        ...
                                        <button
                                            onClick={(e) => {
                                                ...
                                                toast.success(`${wallet.asset} address copied!`);
                                            }}
