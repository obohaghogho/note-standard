    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={cn(
                    "p-1.5 md:p-2 rounded-full hover:bg-white/5 transition-all relative group",
                    isOpen ? "bg-white/10 text-white" : "text-gray-400 hover:text-white",
                    wiggle && "animate-wiggle"
                )}
            >
                <Bell size={18} className={cn("md:w-5 md:h-5", unreadCount > 0 && "text-primary")} />
                {unreadCount > 0 && (
                    <span className="absolute top-1 right-1 min-w-[14px] md:min-w-[16px] h-3.5 md:h-4 bg-red-500 text-white text-[9px] md:text-[10px] font-bold rounded-full flex items-center justify-center ring-1 md:ring-2 ring-black px-1">
