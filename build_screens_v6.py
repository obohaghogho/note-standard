...
def build_v6_overview():
    ...
    path = os.path.join(screens_dir, "v6_wallet_overview.png")
    canvas.save(path)
    print(f"Saved {path}")
...
def build_v6_ngn():
    ...
    path = os.path.join(screens_dir, "v6_ngn_wallet.png")
    canvas.save(path)
    print(f"Saved {path}")
...
def build_v6_usd():
    ...
    draw.text((120, 585), "Global Dollar Account • Active", fill=(16, 185, 129, 255), font=font_small)

    # Action Buttons
    draw.rounded_rectangle([(80, 720), (520, 830)], radius=20, fill=(16, 185, 129, 255))
    draw.text((190, 755), "Add USD", fill=(255, 255, 255, 255), font=font_btn)
    ...
    # Tx 1
    draw.rounded_rectangle([(80, 940), (1000, 1100)], radius=20, fill=(15, 23, 42, 255))
    draw.text((120, 970), "Global Wire Deposit", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((120, 1020), "Successful • Dollar Account", fill=(148, 163, 184, 255), font=font_xs)
    draw.text((750, 970), "+ $ 750.00", fill=(16, 185, 129, 255), font=font_bold)

    path = os.path.join(screens_dir, "v6_usd_wallet.png")
    canvas.save(path)
    print(f"Saved {path}")
...
def build_v6_ghs():
    ...
    draw.text((120, 585), "MTN / Telecel MoMo Active", fill=(16, 185, 129, 255), font=font_small)

    # Action Buttons
    draw.rounded_rectangle([(80, 720), (520, 830)], radius=20, fill=(234, 179, 8, 255))
    draw.text((180, 755), "Deposit GHS", fill=(15, 23, 42, 255), font=font_btn)

    draw.rounded_rectangle([(560, 720), (1000, 830)], radius=20, fill=(51, 65, 85, 255), outline=(234, 179, 8, 255), width=2)
    draw.text((680, 755), "Send MoMo", fill=(255, 255, 255, 255), font=font_btn)

    path = os.path.join(screens_dir, "v6_ghs_wallet.png")
    canvas.save(path)
    print(f"Saved {path}")

# -----------------------------------------------------------------------------
# SCREEN 5: PAYOFF — INTEGRATED CHAT & SUPPORT DASHBOARD
# -----------------------------------------------------------------------------
def build_v6_payoff_chat():
    canvas = Image.new("RGBA", (width, height), (15, 23, 42, 255))
    draw = ImageDraw.Draw(canvas)
    draw_header(draw, "NoteStandard Chat & Support")

    draw.rounded_rectangle([(40, 230), (1040, 2080)], radius=36, fill=(30, 41, 59, 240), outline=(99, 102, 241, 255), width=3)

    # Header Card
    draw.rounded_rectangle([(80, 270), (1000, 450)], radius=24, fill=(24, 34, 56, 255), outline=(99, 102, 241, 255), width=2)
    draw.text((120, 300), "LIVE INTEGRATED CHAT", fill=(129, 140, 248, 255), font=font_bold)
    draw.text((120, 360), "Realtime Messaging & Specialist Support", fill=(148, 163, 184, 255), font=font_small)

    # Chat Message Bubble 1 (User)
    draw.rounded_rectangle([(250, 490), (980, 640)], radius=24, fill=(99, 102, 241, 255))
    draw.text((280, 520), "Can I receive USD and transfer to NGN?", fill=(255, 255, 255, 255), font=font_mid)
    draw.text((280, 580), "9:42 AM • Sent", fill=(224, 231, 255, 255), font=font_xs)

    # Chat Message Bubble 2 (NoteStandard Specialist Response)
    draw.rounded_rectangle([(80, 680), (840, 880)], radius=24, fill=(15, 23, 42, 255), outline=(51, 65, 85, 255), width=2)
    draw.text((110, 710), "NoteStandard Specialist", fill=(52, 211, 153, 255), font=font_bold)
    draw.text((110, 765), "Yes! Instant multi-currency wallets.", fill=(240, 243, 248, 255), font=font_mid)
    draw.text((110, 820), "9:42 AM • Delivered", fill=(148, 163, 184, 255), font=font_xs)

    # Multi-currency quick cards inside chat view
    draw.text((80, 930), "CONNECTED WALLETS", fill=(255, 255, 255, 255), font=font_bold)

    # NGN Card
    draw.rounded_rectangle([(80, 990), (1000, 1140)], radius=20, fill=(24, 34, 56, 255), outline=(99, 102, 241, 255), width=2)
    canvas.paste(flag_ngn, (110, 1035), flag_ngn)
    draw.text((190, 1015), "NGN Wallet", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((190, 1070), "₦ 1,850,000.00", fill=(129, 140, 248, 255), font=font_mid)

    # USD Card
    draw.rounded_rectangle([(80, 1170), (1000, 1320)], radius=20, fill=(18, 48, 38, 255), outline=(16, 185, 129, 255), width=2)
    canvas.paste(flag_usd, (110, 1215), flag_usd)
    draw.text((190, 1195), "USD Wallet", fill=(255, 255, 255, 255), font=font_bold)
    draw.text((190, 1250), "$ 1,200.00", fill=(52, 211, 153, 255), font=font_mid)

    path = os.path.join(screens_dir, "v6_payoff_chat.png")
    canvas.save(path)
    print(f"Saved {path}")
