...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_unread ON messages (conversation_id) WHERE status != 'read';

-- 3. Optimization for sequence number lookups (used by deduplication and delta syncs).
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_sequence ON messages (conversation_id, sequence_number DESC);
