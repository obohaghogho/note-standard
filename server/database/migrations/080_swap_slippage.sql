-- ============================================================================
-- Migration 080: ADD SLIPPAGE PROTECTION TO SWAP QUOTES
-- ============================================================================
-- Purpose:
--   1. Add slippage_tolerance to swap_quotes
--   2. Update execute_swap_from_quote RPC to verify price hasn't drifted
--      beyond the user's selected slippage tolerance.
-- ============================================================================

BEGIN;

-- 1. Add slippage_tolerance column to swap_quotes
ALTER TABLE public.swap_quotes ADD COLUMN IF NOT EXISTS slippage_tolerance NUMERIC DEFAULT 0.005; -- 0.5% default

-- 2. Update RPC to accept and log execution market price 
--    (In a real production system, the RPC might call an oracle or we pass the current price into it as a parameter for strict verification)
--    Since Supabase RPC cannot easily make async HTTP calls to CoinGecko natively during execution without pg_net extensions,
--    we simulate slippage verification by requiring the backend to pass the *current_market_rate* to the execution RPC.

DROP FUNCTION IF EXISTS public.execute_swap_from_quote(UUID, TEXT);
...
BEGIN
    ...
    -- STEP 1.2: SLIPPAGE VERIFICATION (NEW)
    -- Compare locked quote rate vs current exact market rate
    -- Ensure price hasn't moved unfavorably beyond slippage_tolerance
