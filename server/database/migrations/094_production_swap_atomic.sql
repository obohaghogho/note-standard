-- Migration 094: Production Atomic Swap Execution
-- This RPC implements the exact sequence requested: 
-- Calculate -> Execute -> Update Balances -> Record Transaction (including Swaps/Fees tables)
...
BEGIN
    ...
    -- 3. Calculate Fees (Production Rule: 6% Admin / 0.5% Referrer / 1% Reward)
    ...
    -- B. Credit User Net (Target Asset)
    UPDATE public.wallets_store 
    ...
    IF v_admin_wallet_id IS NULL THEN
        SELECT id INTO v_admin_wallet_id FROM public.wallets_store w JOIN public.profiles p ON w.user_id = p.id 
    ...
    -- 6. RECORD TRANSACTIONS (Audit Ledger)
    ...
    -- 7. POPULATE SWAPS & FEES TABLES (New Production Requirement)
