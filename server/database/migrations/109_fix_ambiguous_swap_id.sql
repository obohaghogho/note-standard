-- Migration 109: Fix ambiguous "id" in execute_production_swap
-- The previous query SELECT id INTO v_admin_wallet_id 
-- from wallets_store w JOIN profiles p failed because both have 'id'

CREATE OR REPLACE FUNCTION public.execute_production_swap(
    p_quote_id UUID,
    p_idempotency_key TEXT DEFAULT NULL,
    p_admin_rate NUMERIC DEFAULT 0.06,
    p_partner_rate NUMERIC DEFAULT 0.01,
    p_referrer_rate NUMERIC DEFAULT 0.005
) RETURNS UUID AS $$
DECLARE
    v_quote          RECORD;
    v_tx_id          UUID;
...
BEGIN
    ...
    IF v_admin_wallet_id IS NULL THEN
        -- FIXED: Use w.id instead of ambiguous id
