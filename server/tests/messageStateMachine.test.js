...
runTest('TEST 4 — Sender Reload State Persistence', () => {
    const preReloadMsg = { id: 'uuid-104', event_id: 'evt-104', status: 'delivered', delivered_at: '2026-08-10T00:00:00Z' };
    const dbFetchedMsg = { id: 'uuid-104', event_id: 'evt-104', delivered_at: '2026-08-10T00:00:00Z', read_at: null };
...
runTest('TEST 5 — Recipient Reconnect Monotonic Safety', () => {
    let msg = { id: 'uuid-105', event_id: 'evt-105', status: 'delivered', delivered_at: '2026-08-10T00:00:00Z' };

    // Reconnect buffer replays initial 'sent' message payload
    const staleReplay = { id: 'uuid-105', event_id: 'evt-105', status: 'sent', delivered_at: null };
...
runTest('TEST 6 — Duplicate Delivery ACK Idempotency', () => {
    let msg = { id: 'uuid-106', event_id: 'evt-106', status: 'sent' };
    const ack = { id: 'uuid-106', event_id: 'evt-106', delivered_at: '2026-08-10T00:00:00Z', status: 'delivered' };
...
runTest('TEST 8 — Multi-Device Session State Convergence', () => {
    let msg = { id: 'uuid-108', event_id: 'evt-108', status: 'sent' };

    // Device A sends delivery ACK
    msg = mergeMessageMonotonic(msg, { id: 'uuid-108', event_id: 'evt-108', delivered_at: '2026-08-10T00:00:01Z', status: 'delivered' }, 'socket').merged;
    assertEqual(msg.status, 'delivered', 'Device A ACK upgrades to DELIVERED');

    // Device B sends read event
    msg = mergeMessageMonotonic(msg, { id: 'uuid-108', event_id: 'evt-108', read_at: '2026-08-10T00:00:05Z', status: 'read' }, 'read').merged;
...
runTest('TEST 9 — Network Interruption Re-sync', () => {
    let msg = { id: 'uuid-109', event_id: 'evt-109', status: 'sending' };

    // Network drops during send... HTTP fails, status becomes 'sent' via fallback
    msg = mergeMessageMonotonic(msg, { id: 'uuid-109', event_id: 'evt-109', status: 'sent' }, 'http').merged;
    assertEqual(msg.status, 'sent', 'Status after fallback');

    // Offline queue replays ACK upon reconnection
    msg = mergeMessageMonotonic(msg, { id: 'uuid-109', event_id: 'evt-109', delivered_at: '2026-08-10T00:00:15Z', status: 'delivered' }, 'socket').merged;
