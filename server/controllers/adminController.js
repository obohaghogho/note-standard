...
exports.getStats = async (req, res) => {
    try {
        ...
        const [
            ...
            serviceSupabase.from('conversations').select('*', { count: 'exact', head: true }).eq('chat_type', 'support').eq('support_status', 'open'),
            serviceSupabase.from('conversations').select('*', { count: 'exact', head: true }).eq('chat_type', 'support').eq('support_status', 'pending'),
            serviceSupabase.from('profiles').select('*', { count: 'exact', head: true }).eq('is_online', true)
        ]);
        ...
        const { data: allNotes } = await serviceSupabase.from('notes').select('owner_id');
        const noteCounts = {};
        allNotes.forEach(n => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const notesCount = recentNotes.filter(n => n.created_at.startsWith(dateStr)).length;
            const usersCount = recentUsers.filter(u => u.created_at.startsWith(dateStr)).length;
        ...
        const sevenDaysAgoDate = new Date(today); sevenDaysAgoDate.setDate(today.getDate() - 7);
        const fourteenDaysAgoDate = new Date(today); fourteenDaysAgoDate.setDate(today.getDate() - 14);
        ...
        try {
            const [
                ...
                serviceSupabase.from('profiles').select('*', { count: 'exact', head: true }).gte('created_at', sevenDaysAgoDate.toISOString()),
                serviceSupabase.from('profiles').select('*', { count: 'exact', head: true }).gte('created_at', fourteenDaysAgoDate.toISOString()).lt('created_at', sevenDaysAgoDate.toISOString()),
                serviceSupabase.from('notes').select('*', { count: 'exact', head: true }).gte('created_at', sevenDaysAgoDate.toISOString()),
                serviceSupabase.from('notes').select('*', { count: 'exact', head: true }).gte('created_at', fourteenDaysAgoDate.toISOString()).lt('created_at', sevenDaysAgoDate.toISOString()),
                serviceSupabase.from('conversations').select('*', { count: 'exact', head: true }).eq('chat_type', 'support').eq('support_status', 'resolved'),
                serviceSupabase.from('conversations').select('*', { count: 'exact', head: true }).eq('chat_type', 'support')
            ]);
            ...
            responseData.growthRate = calculateGrowth(usersLast7 || 0, usersPrior7 || 0);
            responseData.noteGrowth = calculateGrowth(notesLast7 || 0, notesPrior7 || 0);
            responseData.chatRetention = (totalSupportChats ? Math.round((resolvedChats / totalSupportChats) * 100) : 0) + '%';
        } catch (e) {
...
exports.getUsers = async (req, res) => {
    try {
        ...
        let query = serviceSupabase
            .from('profiles')
            .select('id, username, email, full_name, avatar_url, role, status, is_online, last_seen, created_at', { count: 'exact' });
...
exports.getUserNotes = async (req, res) => {
    try {
        ...
        const { data: notes, error, count } = await serviceSupabase
            .from('notes')
            .select('id, title, is_private, is_favorite, tags, created_at, updated_at', { count: 'exact' })
            .eq('owner_id', id)
...
exports.updateChatStatus = async (req, res) => {
    try {
        ...
        // Log the action
        await logAdminAction(req, 'update_support_status', 'conversation', id, { status: support_status });
...
exports.joinSupportChat = async (req, res) => {
    try {
        ...
        // Log the action
        await logAdminAction(req, 'join_support_chat', 'conversation', id, { role: 'admin' });
...
exports.exportChatTranscript = async (req, res) => {
    try {
        ...
        const fileName = `transcript-${id.slice(0, 8)}-${new Date().toISOString().split('T')[0]}`;
        ...
        if (format === 'pdf') {
            ...
            doc.text(`Participants: ${conversation.members.map(m => m.profile.username).join(', ')}`);
            doc.moveDown();
            ...
            messages.forEach(msg => {
                ...
                doc.fontSize(10).fillColor('gray').text(`${time} - ${msg.sender.username}:`, { continued: true });
...
exports.getAutoReplySettings = async (req, res) => {
    try {
        ...
        res.json(data || { enabled: false, message: '', start_hour: '18:00', end_hour: '09:00', timezone: 'UTC' });
    } catch (err) {
...
exports.updateAutoReplySettings = async (req, res) => {
    try {
        ...
        await logAdminAction(req, 'update_auto_reply', 'settings', data.id, { enabled });
...
exports.createBroadcast = async (req, res) => {
    try {
        ...
        // Log the action
        await logAdminAction(req, 'create_broadcast', 'broadcast', data.id, { title });
...
exports.deleteBroadcast = async (req, res) => {
    try {
        ...
        // Log the action
        if (broadcast) {
            await logAdminAction(req, 'delete_broadcast', 'broadcast', id, { title: broadcast.title });
        }
    ...
    try {
        ...
        if (error) {
            ...
            if (error.message?.includes('does not exist') || error.code === '42P01') {
                console.warn('[Admin] system_settings table missing, returning defaults');
                return res.json({ 
...
exports.updateSystemSettings = async (req, res) => {
    try {
        const serviceSupabase = getServiceSupabase();
        const { system_name, maintenance_mode, registration_status, admin_2fa_enabled } = req.body;
        ...
        if (error) {
            ...
            return res.status(400).json({ error: 'Failed to update settings: ' + error.message });
        }
        ...
        res.status(400).json({ error: 'System error during update: ' + err.message });
    }
};
