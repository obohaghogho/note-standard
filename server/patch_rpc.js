...
async function fixRPC() {
  console.log("Removing Max Swap limit from RPC...");
  const { error } = await supabase.rpc("exec_sql", {
    query: `
        ...
        BEGIN
            ...
            -- REMOVED MAX SWAP DB CHECK AS IT BLOCKS NGN/JPY
  ...
  if (error) {
    console.error(
      "Exec SQL Failed! (Supabase RPC 'exec_sql' might not exist, which is expected on standard Supabase). We will fix the SQL manually.",
    );
  } else {
    console.log("Success!");
  }
