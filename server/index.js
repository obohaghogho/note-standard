...
io.use(async (socket, next) => {
    try {
        ...
        if (authError || !user) {
            console.error('[Socket Auth] Invalid token:', authError?.message || 'User not found');
        ...
        try {
            ...
            console.warn('[Socket Auth] Profile fetch failed or timed out:', profileErr.message);
...
setInterval(async () => {
    try {
        ...
        if (ghosts && ghosts.length > 0) {
            ...
            console.log(`[Presence] Cleaning up ${ghostIds.length} ghost users`);
            ...
            ghostIds.forEach(id => {
                io.emit('user_online', { userId: id, online: false, lastSeen: new Date().toISOString() });
...
io.on('connection', async (socket) => {
    ...
    if (socket.userProfile?.role === 'admin' || socket.userProfile?.role === 'support') {
    ...
    socket.on('join_room', async (conversationId) => {
        try {
            ...
            } else if (socket.userProfile?.role === 'admin' || socket.userProfile?.role === 'support') {
    ...
    socket.on('mark_read', async ({ conversationId, messageId }) => {
        socket.to(conversationId).emit('message_read', { conversationId, messageId, readBy: userId });
    });
    ...
    socket.on('admin_viewing_chat', ({ conversationId, adminName }) => {
        ...
        socket.to(conversationId).emit('admin_presence_update', { conversationId, adminId: userId, adminName, status: 'viewing' });
    });

    socket.on('admin_leaving_chat', ({ conversationId }) => {
        socket.to(conversationId).emit('admin_presence_update', { conversationId, adminId: userId, status: 'left' });
        socket.currentChatId = null;
    });

    // --- WebRTC Signaling ---
    socket.on('call:init', ({ to, type, conversationId }) => {
        console.log(`[WebRTC] Call Init from ${userId} to ${to} (${type})`);
        io.to(to).emit('call:incoming', { from: userId, type, conversationId });
    });

    socket.on('call:ready', ({ to }) => {
        console.log(`[WebRTC] Recipient ${userId} is ready for offer to ${to}`);
    ...
    socket.on('disconnect', async () => {
        ...
        if (socket.currentChatId && socket.adminName) {
            io.to(socket.currentChatId).emit('admin_presence_update', { conversationId: socket.currentChatId, adminId: userId, status: 'left' });
        }
...
const subscriptionRoutes = require(path.join(__dirname, 'routes', 'subscription'));
