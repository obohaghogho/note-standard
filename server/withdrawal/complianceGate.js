...
class ComplianceGate {
   ...
   * @param {number} params.amount
   * @param {string} params.currency
   * @returns {Promise<{ allowed: boolean, errorCode?: string, reason?: string }>}
   */
  async evaluateConversion({ userId, amount, currency }) {
    if (!userId || !amount) {
      return {
        allowed: false,
        errorCode: "INVALID_PARAMETERS",
    ...
    const { data: profile, error: profileErr } = await supabase
      .from("profiles")
      .select("id, email, is_verified, kyc_level, status")
      .eq("id", userId)
      .single();
